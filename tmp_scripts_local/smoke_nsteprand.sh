...
~/bin/micromamba run -p ~/micromamba/envs/teacher311 python learn/indexed_attention/train_prompt_response.py \
  ...
  --tokenizer qwen35 --d_model 256 --n_head 4 --n_step 4 --n_step_train_max 8 --use_ff --batch_size 8 \
  --lr 1e-4 --seed 0 --num_workers 2 --max_steps 12 --val_every 12 --val_batches 2 \
  --save_checkpoint_path checkpoints/smoke_nsteprand_test.pt \
  2>&1
echo SMOKE_NSTEPRAND_DONE
