...
- **Test d'auto-appariement, décisif** : sur 300 tokens aléatoires isolés (sans registre, sans hiérarchie, sans distracteurs d'épisode — juste `q_proj(\text{embed}(x))` vs `k_proj(\text{embed}(x)+\text{source\_bias})`), le bon appariement $x \leftrightarrow x$ est classé en moyenne **21ᵉ sur 300** (hasard : 150) — **au-dessus du hasard, une vraie capacité de matching par contenu existe**, même si ce n'est jamais l'appariement n°1 exact (top-1 = 0%).

**Conclusion, en tension avec les tests précédents** : la capacité de matching par contenu de `q_proj`/`k_proj` existe et fonctionne (modestement) en isolation, mais **s'effondre spécifiquement** dans le contexte restreint à 4 candidats d'un épisode réel (où les tests précédents donnaient un rang **en dessous** du hasard). Ni un manque de capacité générale, ni un collapse de poids, ni une domination du biais/registre n'expliquent cette différence — l'interaction précise (peut-être quelque chose de spécifique à la structure des 4 candidats d'un même épisode, ou aux niveaux compressés au-dessus du niveau 0) reste **non résolue**. Prochain test naturel (pas encore fait) : refaire le test d'auto-appariement en tirant les candidats **de la même façon que `KBChainDataset`** (quadruplets structurés, pas 300 tokens uniformément aléatoires) pour isoler si c'est la petite taille du groupe ou sa structure spécifique qui casse le matching. **Point d'arrêt de ce round de diagnostic** — rendements décroissants au-delà de ce point sans nouvelle méthode ; à reprendre plus tard si priorisé.
