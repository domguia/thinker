...
**Ce que ça clôt** : le plateau de composition `n_hops=2` qui a résisté tout au long de cette session — sweep `N_step` (écarté), sweep LR à budget complet (écarté), `use_ff` (neutre, hors-sujet on le sait maintenant), `n_register` (neutre/pire, hors-sujet), sharpening/normalisation de requête (échec, mauvaise hypothèse) — était en réalité un problème de **signal d'entraînement**, pas de capacité architecturale ni d'hyperparamètre. Une supervision contrastive bon marché, sans paramètre supplémentaire, suffit à le résoudre complètement à cette échelle. **Prochaine étape** : confirmation à plus grande échelle (grille GPU en cours, experiment-manager, `d_model=128`, 3 graines, ~2h32 de marge A100) avant de considérer cette piste comme définitivement validée et de l'intégrer par défaut dans l'entraînement `Thinker` sur les tâches multi-sauts.
