...
**Réponse partielle à la question posée** : le découplage seul, à ce jour et sur cette grille, ne bat pas le raccourci à `n_hops=2` (Étape 1, corrigé) ; découplage **+** supervision du bon nœud le bat très nettement, sur toutes les seeds. La supervision n'est donc **pas (encore) redondante** — mais la conclusion définitive attend le balayage LR en cours (ci-dessous) sur le découplage seul : si un LR mieux calibré suffit à lui seul à battre le raccourci, la supervision devient un correctif de LR déguisé, pas un ajout structurel nécessaire ; si le balayage LR échoue malgré une fenêtre correctement explorée, la supervision est validée comme nécessaire à cette échelle, indépendamment du LR. Ne pas trancher avant ce résultat.
