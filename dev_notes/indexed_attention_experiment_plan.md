...
**[RÉSULTAT 2026-09-14] — tranché, net.** 12/12 cellules à 100% : `n_facts=64` (`depth=1` : 100/100/100 ; `depth=4` natif : 100/100/100), `n_facts=256` (`depth=1` : 100/100/100 ; `depth=5` natif : 100/100/100). **`depth=1` égale la profondeur native aux deux échelles, une fois le curriculum appliqué** — la décision "démarrer l'intégration texte réel avec `depth=1`, sans index" est maintenant appuyée par des preuves réelles à `n_facts=64/256`, pas seulement l'extrapolation depuis `n_facts=16`. Rien à réviser ; confirmé plutôt qu'infirmé.
...
**[RÉSULTAT 2026-09-14] Étape 4 terminée — premier chaînage réel confirmé jusqu'à `n_hops=4`.** Générateur durci (`max_facts=8`, `n_distractors=8-n_hops`), découplé, LR resweepé : **les 3 `n_hops` (2, 3, 4) battent nettement le raccourci sur toutes les seeds, marge +0,78 à +0,87, aucune exception.** C'est la confirmation la plus propre à ce jour de la thèse centrale du projet (le chaînage itératif compose réellement, pas seulement à 1-2 sauts).

**LR trouvé (`3e-4`, `1e-4` marchent aussi), mais fenêtre extrêmement étroite et non monotone** : `6e-4`, `1,2e-3`, `3e-3` échouent tous franchement — pas de dégradation progressive, un mur juste au-dessus de `3e-4`. À retenir comme signature répétée de ce mécanisme (même motif que `n_hops=2` avant durcissement) plutôt qu'une anomalie ponctuelle : la fenêtre de LR stable du découplage semble systématiquement étroite et localisée bas, quelle que soit la taille de KB.

Suite déjà lancée par `experiment-manager` (proactivement, conforme à la table de décision) : Phase 1bis (`use_ff`×`n_register`) à `n_hops∈{3,4}`, 12 runs, résultats à venir.

**[RÉSULTAT 2026-09-14] `pool_n_head`/`k_dim` — image mitigée, un résultat à ne pas ignorer.** 12/12 cellules terminées. À `lr=3e-4` : effet de plafond, les deux variantes à 100%, aucune différenciation. **À `lr=6e-4` : `pool_n_head=4` seul (sans découplage) est robuste sur les 2 seeds (99,2-99,8%)** — c'est exactement le résultat "contraire à la prédiction" que la spec §5.1bis demandait de ne pas ignorer si ça arrivait : le multi-tête seul semble approcher une victoire nette, pas juste une atténuation partielle. `k_dim=128` (avec découplage) est en revanche **incohérent à ce même LR** (100% puis 33,4% selon la seed) — plutôt une instabilité qu'un signal, à l'opposé de ce qu'annonçait le précédent Product-Key Memory. Aucune variante ne survit à `1,2e-3` (même mur que la baseline découplée).

**Pas encore concluant avec 2 seeds/cellule — prochaine étape : plus de seeds, ciblées sur `pool_n_head=4` (shared) à `lr=6e-4` en priorité** (5 seeds supplémentaires, pour confirmer ou infirmer avant de réviser le raisonnement structurel de la spec §5.1bis) ; `k_dim` en second (l'instabilité 100%/33% suggère qu'il faudrait aussi balayer le LR spécifiquement pour cette variante plutôt que de réutiliser les 3 points déjà testés — pas encore fait). Ne pas conclure sur l'un ou l'autre avant ces seeds/ce sweep supplémentaires (règle du résultat ambigu, méthodologie commune).

**[RÉSULTAT 2026-09-14] `pool_n_head=4` à `lr=6e-4`, 7 seeds au total — lecture précédente RETIRÉE, ce n'est pas un résultat robuste.** s0=99,77%, s1=99,16%, s6=94,67% réussissent (marge +0,62 à +0,68) mais s2=32,60%, s3=33,59%, s4=34,18%, s5=21,07% échouent franchement (marge -0,12 à +0,014, aucune net) — **4/7 échecs, résultat bimodal/incohérent d'une seed à l'autre, pas une confirmation.** Ce n'est donc pas le "contraire à la prédiction" qu'on pensait tenir : ça ressemble à la même signature de fenêtre LR étroite mal calée que partout ailleurs sur ce projet (Étape 4 : `3e-4` marche, `6e-4` échoue déjà pour le découplage seul), pas à un effet structurel propre au multi-tête. **Ne pas réviser le raisonnement structurel de la spec §5.1bis sur la base de ce résultat.**

Signal utile en parallèle, sur la même config : `k_dim=128` (découplé) à `lr=4,5e-4` (intermédiaire entre les deux points déjà testés) est **propre sur 2/2 seeds** (98,61%/99,90%, marge +0,65/+0,68) — cohérent avec "fenêtre LR étroite mal calée" pour ce variant aussi, pas une instabilité de mécanisme.

**Prochaine étape (les deux variantes)** : balayage LR dédié et plus fin pour `pool_n_head=4` (ex. `4e-4, 4,5e-4, 5e-4, 5,5e-4`, ≥5 seeds/cellule dès le départ vu la bimodalité déjà observée — 2 seeds ne suffisent plus à faire confiance à une cellule) pour trouver sa vraie fenêtre stable, si elle existe, avant toute conclusion structurelle dans un sens ou l'autre.

**[RÉSULTAT 2026-09-14] Balayage fin terminé (20/20, 5 seeds/LR) — fenêtre stable trouvée, tranché : c'est bien un résultat "contraire à la prédiction", confirmé cette fois.** `lr=4e-4` : **5/5 robuste, 99,6-99,9% sur toutes les graines**, aucune exception. `4,5e-4` (un seul cran au-dessus) montre déjà des échecs complets (23,5%) et partiels — la fenêtre stable est étroite et centrée sur `4e-4`, pas sur `6e-4` (le point testé initialement, hors fenêtre, d'où le bimodal 3/7 vs 4/7 observé au départ, maintenant pleinement expliqué). **Conclusion révisée : `pool_n_head=4` SEUL (sans `decouple_kv`), à son propre LR correctement calé, atteint bien une victoire nette (99,6-99,9%) comparable au découplage complet** — pas juste une atténuation partielle comme le prédisait le raisonnement structurel de la spec §5.1bis. Le multi-tête suffit apparemment à résoudre le conflit K/V par lui-même, sans requête dédiée. **Spec §5.1bis à mettre à jour en conséquence** (fait, voir spec) — implication pratique : `pool_n_head` pourrait être une alternative moins coûteuse à `decouple_kv` (zéro nouveau paramètre) pour ce problème, à garder en tête pour Phase 3/4.

**[RÉSULTAT 2026-09-14] `k_dim=128` (découplé) confirmé de la même façon — même signature, même verdict.** 5/5 graines à `lr=4,5e-4` : 98,6-100% (marge +0,65 à +0,68), parfaitement stable, comme `pool_n_head=4` à `lr=4e-4`. **Confirme que l'instabilité 100%/33,4% observée initialement à `lr=6e-4` était bien une fenêtre LR mal calée pour les deux variantes, pas un effet structurel différent entre elles** — contrairement à l'inquiétude initiale que `k_dim` contredirait le précédent Product-Key Memory (§5.4), rien ici ne le suggère une fois le LR correctement trouvé. **Fil `pool_n_head`/`k_dim` clos** : les deux variantes marchent, chacune a sa propre fenêtre LR étroite (`4e-4` et `4,5e-4` respectivement, ne pas réutiliser l'une pour l'autre). Reste ouvert : lequel des deux (ou les deux combinés) apporte le plus de valeur en pratique — question à trancher si besoin en Phase 3/4, pas prioritaire tant que le texte réel n'a pas encore été testé avec (en cours, voir Phase 3).
...
**Nouvelles variantes, implémentées 2026-09-14 (demande explicite de l'utilisateur), à lancer en petite échelle en parallèle du reste** — `pool_n_head` (pooling multi-tête, §5.1bis) et `k_dim` (asymétrie $\dim(K)<\dim(V)$, §5.4). Toutes deux `[DÉFAUT]` par défaut à l'ancien comportement, testées (46 tests + vérifications manuelles, formes/gradient/NaN), zéro résultat empirique sur tâche pour l'instant.

**Protocole recommandé, sur la cellule déjà propre (`n_hops=2, n_distractors=2, depth=2, block_size=4, decoupled`)** — ne pas réutiliser `lr=3e-4` sans le revérifier, ces flags changent la forme des tenseurs internes/l'échelle des produits scalaires, même sans nouveau paramètre pour `pool_n_head` :
1. **`pool_n_head` seul, SANS `decouple_kv`** (`--shared_kv_pooling --pool_n_head 4` ou 8) : teste directement l'hypothèse de l'utilisateur — le multi-tête peut-il résoudre le conflit K/V structurel "par accident", sans la requête dédiée ? Prédiction (raisonnement §5.1bis) : atténuation partielle au mieux, pas une victoire nette comme le découplage plein. Un résultat qui contredit cette prédiction (victoire nette) serait en soi important à documenter, pas à ignorer.
2. **`pool_n_head` en plus de `decouple_kv=True`** : est-ce que ça aide encore, une fois le conflit structurel déjà levé ? Pertinent surtout pour `n_hops≥3` (Étape 4) plutôt que `n_hops=2` (déjà résolu à 100% sans ce flag) — à combiner avec la grille Étape 4 une fois durcie, pas nécessairement testé isolément à `n_hops=2` en premier.
3. **`k_dim` (ex. `d_model/2`) avec `decouple_kv=True`**, sur `n_hops=2` d'abord (cellule propre) : `k_dim` plus petit devrait rester neutre ou légèrement bénéfique par le précédent Product-Key Memory (la valeur porte l'information, la clé n'a besoin que d'être discriminante) — un résultat contraire (dégradation nette) serait la découverte la plus importante ici (remettrait en question l'application directe du précédent PKM à ce mécanisme).
4. Balayage LR obligatoire avant toute conclusion sur (1)/(3), ≥3 seeds, `margin_over_shortcut` dans chaque rapport comme toujours.

**Ressources** : demande explicite de l'utilisateur — réserver autant de nœuds que possible pour accélérer ces expériences, en parallèle de l'Étape 4 (durcissement du générateur) et de Phase 3 (texte réel), déjà en cours. Les trois chantiers sont indépendants et peuvent tourner simultanément sans se bloquer.

**Nouvelle variante 2026-09-14, importée du chantier toy-memory — `sm_cap` (déjà câblé, `--sm_cap`, zéro nouveau code)** : Exp. 2 du toy model (`n_memory`) a trouvé que `n_memory=1` égale `n_memory=10000` sur `copy` — pas d'accumulation détectée à travers plusieurs emplacements mémoire, le mécanisme fonctionne comme une récurrence simple à cette échelle (`dev_notes/toy_memory_experiment_plan.md`, §5). La SM du `Thinker` principal (buffer par `APPEND`, jamais borné jusqu'ici) est structurellement le même mécanisme — teste directement `sm_cap=1` vs `sm_cap=None` sur la cellule `n_hops=2` déjà résolue (découplé + `lr=3e-4`).
- `sm_cap=1` égale `sm_cap=None` (les deux ≈100%) → même lecture que le toy model : la SM fonctionne comme une récurrence simple ici aussi, pas une vraie mémoire multi-emplacements — résultat important à remonter directement, pas une note de bas de page (le buffer APPEND pourrait être une complexité inutile pour ce type de tâche).
- `sm_cap=1` dégrade nettement → contraste avec le toy model, la SM du Thinker sert réellement à accumuler plusieurs faits — cohérent avec `n_hops≥2` qui a justement besoin de porter le résultat du premier saut jusqu'au second, contrairement au `copy` du toy model qui n'a rien à "combiner", juste à transporter.
- Résultat mitigé/seed-dépendant → pas concluant à cette échelle, ne pas sur-interpréter, éventuellement à re-tester à `n_hops≥3` une fois l'Étape 4 durcie.

**[RÉSULTAT 2026-09-14] `sm_cap=1` égale `sm_cap=None` — premier branchement de la table de décision confirmé.** 6/6 cellules, `n_hops=2, n_distractors=2, lr=3e-4`, 3 seeds : `cap=1` (99,84/100/100%, marge +0,65-0,66) et `cap=none` (100/100/99,98%, marge +0,66-0,68) — **indiscernables**, écart entièrement dans le bruit de seed. Exactement le même signal que Exp. 2 du toy model (`n_memory=1` ≈ `n_memory=10000`) — pas d'accumulation multi-emplacements détectée, ici non plus.

**[NUANCE IMPORTANTE 2026-09-14, retour direct de l'utilisateur] — ne pas lire ce résultat (ni le `disable_sm` à venir) comme "la SM est inutile".** `kb_chain_retrieval.py` est markovien par construction (résoudre le saut $i{+}1$ ne demande que la valeur du saut $i$, jamais rien de plus ancien) — une chaîne ne peut donc **structurellement pas** exiger de garder plusieurs faits distincts simultanément en mémoire, peu importe la qualité du mécanisme testé. Voir spec §9.1 pour l'analyse complète et la tâche correcte à construire (faits indépendants, temporellement séparés, à combiner en sortie) avant de conclure quoi que ce soit sur l'utilité réelle de la SM multi-emplacements. Le rôle architectural de la SM (lever le goulot d'information du seul registre $R$, spec §-1) n'est pas remis en cause par ces résultats — c'est le test qui était mal calibré pour la question, pas le composant qui serait démontré superflu.

**Lecture mécaniste (pourquoi c'est plausible, pas juste une coïncidence)** : le registre $R$ est lui-même une vraie récurrence — il persiste et s'accumule à chaque itération via $R_t = R_{t-1} + \Delta$. Les entrées de la SM (`new_k, new_v = sm_write_proj(R)`) sont dérivées de ce même $R$ à chaque pas — si $R$ porte déjà tout ce qui a été fusionné jusqu'ici, interroger la SM (même sur plusieurs emplacements) revient à re-consulter une fonction de quelque chose que $R$ contient déjà, d'où la redondance.

**Test plus direct implémenté, prêt à lancer** : `Thinker(disable_sm=True)` — saute entièrement l'écriture (`sm_write_proj`) et la lecture ($O_{sm}$ forcé à zéro), pas seulement plafonné à 1 emplacement comme `sm_cap=1` (qui écrit/lit encore un slot). Teste la question plus radicale : la SM apporte-t-elle *quoi que ce soit* au-delà du registre récurrent lui-même ? Combinable avec `disable_kb` (boucle "pure", spec §9 Baseline B poussée à l'extrême). Vérifié : `sm_write_proj`/`sm_q_proj` ne reçoivent aucun gradient quand actif (déconnexion réelle du graphe), 46 tests toujours verts.
- `disable_sm=True` égale `sm_cap=None` sur `n_hops=2` → confirme que la SM entière (pas juste sa capacité) est superflue pour cette tâche à cette échelle — remonte-le directement, c'est un résultat architectural important (possible simplification du Thinker pour les tâches qui n'ont besoin que de transporter, pas d'accumuler plusieurs faits distincts).
- `disable_sm=True` dégrade (même légèrement) par rapport à `sm_cap=None` alors que `sm_cap=1` ne dégradait pas → signal fin mais réel que même un seul slot actif (lu/écrit) apporte quelque chose que le retrait complet du mécanisme ne permet pas — plus surprenant, à creuser (pourquoi 1 slot suffit mais 0 slot ne suffit pas).

**[RÉSULTAT 2026-09-14] `disable_sm` terminé (3/3 seeds) — c'est la seconde branche ci-dessus, en beaucoup plus net qu'attendu.** 1,74% / 1,66% / 1,64%, marge **-0,31 à -0,33 SOUS le raccourci** — pas juste "aucune amélioration", un effondrement complet en dessous même de la solution triviale. Contraste net avec `sm_cap=1` (99,8-100%, aucune dégradation) : passer de "1 emplacement actif" à "0 emplacement" est catastrophique, alors que passer de "illimité" à "1 emplacement" ne changeait rien.

**Lecture, à ne pas confondre avec une validation du "multi-emplacements"** (mention "non concluant" archivée dans `experiment.log.md` comme convenu, `kb_chain_retrieval` reste markovien — voir plus haut et spec §9.1) : ce résultat ne dit toujours rien sur l'utilité de PLUSIEURS emplacements simultanés. Mais il établit autre chose, indépendant de cette question et qui n'était pas acquis d'avance : **dans l'implémentation actuelle, le chemin SM lui-même (même réduit à un seul emplacement lu/écrit) est structurellement nécessaire pour que l'information atteigne $R$** — la KB seule (`o_kb`) ne compense pas son absence, alors qu'on aurait pu imaginer une redondance totale entre les deux chemins. Ce n'est donc pas "la SM est retirable sans coût" (lecture qu'on avait explicitement écartée après le retour de l'utilisateur) — c'est plutôt une confirmation, sur un axe différent de celui visé, que le composant SM porte une partie du signal que rien d'autre ne porte à sa place ici. Cohérent avec, et renforce, le principe architectural du registre $R$ multi-sources (spec §-1).

Pas encore câblé en CLI (`train_kb_chain.py` est le fichier d'`experiment-manager`, à ajouter de son côté comme pour `pool_n_head`/`k_dim`) — fait depuis (résultat ci-dessus obtenu avec).
...
**[DÉCISION 2026-09-14] Baselines A/B/C avant d'agrandir le corpus** — question posée par l'utilisateur : plus pertinent maintenant que d'améliorer le dataset ? Oui, pour deux raisons : (1) on ne sait pas encore si `depth=1`/Thinker apporte quoi que ce soit face à des baselines plus simples — question scientifiquement plus urgente que "plus de données" tant qu'elle n'est pas posée ; (2) les baselines sont 3-4 runs indépendants sur les données déjà prêtes, donc **beaucoup plus facilement parallélisables sur le cluster** qu'une tâche de préparation de données (plutôt CPU/séquentielle) — cohérent avec la consigne de maximiser l'occupation.

**Implémenté** : Baseline A = `--n_step 1` (déjà disponible, aucun code). Baseline B = `Thinker(disable_kb=True)` / `--disable_kb` (nouveau, `core/indexed_thinker_model.py`) — la boucle tourne toujours `n_step` fois mais saute entièrement `HierarchicalMemory.build()`/`attend()` (moins cher, pas juste différent architecturalement), $O_{kb}$ mis à zéro dans la fusion. **Choix de périmètre `[DÉFAUT]`** : désactive seulement la KB, pas la SM (qui vient de la trajectoire de la boucle elle-même, pas d'une source externe — lit comme faisant partie du "mécanisme de boucle" au sens de la question posée par cette baseline, pas de "la mémoire"). Vérifié : `memory.*` ne reçoit aucun gradient quand `disable_kb=True` (bien déconnecté du graphe), 46 tests toujours verts, testé de bout en bout. Baseline C = pas réimplémentée ici — `learn/distill/train_sft.py` fait déjà exactement ça (transformer dense standard) et consomme le **même format JSONL** (`"text"` par ligne) que `RealTextWindowDataset` — réutiliser directement `data/distill/general_realtext/{train,val}.jsonl` (déjà généré), pas besoin d'une deuxième préparation de données.

**Prochaine étape, table de décision** :
- Lance en parallèle, sur le corpus déjà prêt : (1) config principale `depth=1` à `lr=3e-3`, budget généreux (le wraparound d'époque ne limite plus artificiellement) ; (2) Baseline A (`--n_step 1`) ; (3) Baseline B (`--disable_kb`) ; (4) Baseline C via `train_sft.py` sur le même JSONL, taille de modèle approximativement comparable en nombre de paramètres.
- `depth=1` bat nettement B (sans mémoire) et se rapproche de C (borne dense) → la mécanique Thinker apporte quelque chose sur du texte réel, continuer vers Phase 4 (distillation).
- `depth=1` ≈ B → le gain ne vient pas de la mémoire externe à cette échelle/ce corpus — creuser pourquoi avant de continuer (dimensionnement ? corpus trop petit pour que la mémoire externe serve à quelque chose, comme `use_ff` sur les tâches synthétiques ?).
- Toutes les configs (principale, A, B) plafonnent au même niveau, y compris C → suspecte alors le corpus (2700 documents TinyStories, petits) plutôt que l'architecture — c'est à ce moment-là qu'agrandir le dataset devient prioritaire, pas avant.

**[RÉSULTAT 2026-09-14] Les quatre configs tombées — ne rentre dans AUCUNE des trois branches ci-dessus telle quelle, ne pas conclure encore.** Principale (`depth=1`, `lr=3e-3`, 100k pas) `final_loss=1,3517` ; Baseline A (`n_step=1`) `1,512` ; Baseline B (`disable_kb`) `2,292` ; Baseline C (transformer dense standard, 12,5M paramètres) `~0,36` (moyenne des 25 derniers points, step 47000) **mais interrompue par expiration de walltime avant son propre résumé final — pas un chiffre à budget égal avec les trois autres (100k pas), donc pas directement comparable en l'état.**

Premier niveau de lecture, cohérent et rassurant pour l'architecture : **principale bat B (1,35 < 2,29)** — la KB apporte quelque chose sur du texte réel, pas juste du bruit — **et bat A (1,35 < 1,51)** — boucler avec mémoire bat un seul passage. Les deux comparaisons qui testent directement les composants Thinker vont dans le sens attendu.

**Ce qui ne va PAS dans le sens attendu et n'est PAS encore interprétable : l'écart à C est énorme (0,36 vs 1,35)**, mais avant de le lire comme "l'attention dense écrase l'approche Thinker sur texte réel", trois confondants doivent être vérifiés — même discipline que pour `n_hops`/LR ailleurs dans ce plan (ne jamais comparer avant d'avoir écarté un biais de protocole) :
1. **Le nombre de pas/tokens vus n'est pas le même** (100k pour principale/A/B vs. 47k avant coupure pour C, walltime insuffisant) — un nombre de pas différent invalide la comparaison brute des pertes, indépendamment de tout le reste.
2. **La définition de la perte doit être vérifiée identique** entre `train_real_text.py` (principale/A/B) et `train_sft.py` (C) — même réduction (moyenne par token, mêmes positions supervisées), pas une perte calculée différemment (ex. seulement sur une sous-séquence, ou une définition de fenêtre différente) qui rendrait les deux nombres non comparables même à pas égal.
3. **La parité de taille** ("approximativement comparable en nombre de paramètres", décidé plus haut) doit être confirmée a posteriori — 12,5M pour C vs. le compte réel de la config principale, pas juste supposée.

**Table de décision révisée, à trancher avant Phase 4** :
- Les trois confondants vérifiés OK (pas/tokens égaux ou normalisés, perte définie pareil, paramètres comparables) et l'écart persiste → résultat réel et important, à documenter comme tel (l'attention dense a un avantage net à cette échelle sur ce corpus) — ne bloque pas Phase 4 mais change son cadrage (la distillation viserait alors à rapprocher Thinker de C, pas juste à accélérer l'entraînement).
- Un des trois confondants explique l'écart (ex. C avait vu moins de la moitié des tokens et son propre `train_sft.py` log montre une pente encore descendante à l'arrêt) → relancer C avec un budget de pas/walltime réellement égal à la principale, pas de conclusion avant ce chiffre propre.

**[RÉSULTAT 2026-09-14] Confondant (2) confirmé réel, pas juste une prudence excessive — les deux tâches ne sont pas les mêmes.** `train_real_text.py` calcule la perte uniquement sur les `t_tgt=32` dernières positions d'une fenêtre glissante (contexte de `n_ctx` tokens tenu via KB/registre) ; `train_sft.py` (chemin réellement emprunté par C, sans `--teacher_targets`/`--mup`) calcule la perte causale standard HF sur **tout** le document tokenisé (jusqu'à `block_size=256`), avec self-attention directe sur tout le préfixe. C prédisait donc une tâche structurellement plus facile (mots très prédictibles dans un même bloc, self-attention complète) — l'écart de perte (0,36 vs 1,35) peut s'expliquer entièrement par cette différence de tâche, indépendamment de tout mérite architectural. **Ne pas citer ce chiffre comme "l'attention dense bat Thinker" sous quelque forme que ce soit tant que ce point n'est pas corrigé.**

**Fix demandé** (envoyé à `experiment-manager`) : nouveau mode dans `train_sft.py` réutilisant directement `data.real_text_windows.RealTextWindowDataset` (mêmes `n_ctx`/`t_local`/`t_tgt`/`stride` que la config principale) — `input_ids = cat([kb_tokens, target_input])`, `labels = cat([full(-100, n_ctx), labels_cible])` (réutilise telle quelle la ligne de perte existante avec `ignore_index=-100`, le masquage fait tout le travail), `attention_mask` reprenant `kb_leaf_mask` pour les contextes courts. Relance ensuite à budget de PAS égal (100k), pas juste un plus grand walltime.

**Asymétrie à garder en tête pour l'interprétation finale, non corrigée délibérément** : la config principale porte $R$ d'une fenêtre à l'autre au sein d'un document (carry-over), donc elle voit potentiellement plus loin que les `n_ctx` derniers tokens sur un document long — C, lui, ne voit jamais que sa propre fenêtre isolée. Si la principale l'emporte quand même après ce fix, une partie de l'écart pourrait venir de cette portée plus longue plutôt que du mécanisme de compression lui-même — à noter dans la lecture finale, pas à retirer (ce serait retirer un avantage réel et voulu de l'architecture).
- Perte définie différemment entre les deux scripts → pas de comparaison directe possible du tout tant que ce n'est pas harmonisé ; ne pas bricoler un facteur de correction ad hoc, aligner les deux calculs de perte proprement.

**[RÉSULTAT FINAL 2026-09-14] Les trois confondants levés, l'écart persiste et il est large.** Budget de pas égal (100k), perte alignée (`--real_text_windows`), paramètres proches (12,52M vs 13,09M, ~4,5% d'écart). **Comparatif propre** : principale `1,3517` ; A `1,512` ; B `2,292` ; **C `0,4137`**. C bat la principale par un facteur ~3,3× sur la perte — et rappel, l'asymétrie du carry-over de $R$ jouait en faveur de la principale, pas de C, donc elle ne peut pas expliquer l'écart (si quoi que ce soit, l'écart réel du mécanisme lui-même serait encore plus grand sans cet avantage).

**Lecture correcte, à ne pas confondre avec "l'attention dense bat l'architecture" — la portée du résultat compte autant que le résultat lui-même.** `n_ctx=256` est une fenêtre qui tient trivialement, en entier, dans une seule couche d'attention dense standard — ce test compare donc "compression avec perte d'info" contre "accès direct sans aucune compression, à un coût que le contexte est encore assez petit pour se permettre." C'est exactement le régime où la compression ne PEUT que perdre : sa valeur ajoutée (spec §-1, motivation du bottleneck du registre central) est de permettre l'accès à une KB bien plus grande que ce qu'une fenêtre d'attention dense pourrait se permettre — pas de battre l'attention dense sur un contexte qui tient déjà confortablement dedans. Ce résultat quantifie donc **le coût réel de la compression à cette échelle** (un fait important et jusqu'ici jamais mesuré), pas une invalidation du pari architectural. Cohérent avec l'avertissement de l'utilisateur du 2026-09-14 : ne pas conclure qu'un composant motivé par principe (ici, l'indexation hiérarchique pour la mise à l'échelle) est superflu sur la base d'un résultat à petite échelle qui ne teste structurellement pas la question pour laquelle il a été conçu.

**Suivi qui testerait la vraie proposition de valeur, pas bloquant, à prioriser après Phase 4** : refaire la même comparaison à un `n_ctx` bien plus grand — assez grand pour que l'attention dense devienne coûteuse/peu pratique (où le coût quadratique commence à peser), et voir si l'écart RELATIF se réduit à mesure que l'échelle grandit (prédiction du pari architectural) ou reste proportionnellement le même (ce qui serait, cette fois, un signal réellement préoccupant).
...
#### Étape 1 exécutée : S0 confirmé sur les vraies données, signal d'attribution absent **[RÉSULTAT 2026-09-19/20, exécuté par experiment-manager sur `paradoxe-27`]**

**S0 (`convert_ffn_to_kv.py`, couches 0/4/8/11/15) : PASS parfait, bit-à-bit** — `max_abs_err=0,0`, `max_rel_err=0,0` sur les 5 couches, aucune exception. Confirme sur les vrais poids OLMo-2-1B (pas seulement synthétiques) que la forme bilinéaire à porte (`silu(x·gate_i) * (x·up_i)`, cf. docstring de `convert_ffn_to_kv.py`) reconstruit exactement la FFN SwiGLU réelle. Harnais validé, `S1`/le curriculum peuvent s'appuyer dessus sans réserve.

**Étape 1 (`diagnose_layer_source_attribution.py`, couches 0/8/15) : aucun signal, dans les deux modes de requête.** `mass_on_own_region` reste à 0,330-0,341 (`identity`, Q=x — le mode principiel) et 0,326-0,335 (`q_proj`), statistiquement indiscernable du contrôle chance (labels mélangés, ~0,333-0,334) et du plancher uniforme (1/3). Aucune couche, aucun mode ne montre de séparation. Détail complet : `runs/olmo_ffn_geometry/{s0_conversion_check,layer_source_attribution}.json` (home Rennes), `dev_notes/experiment.log.md` (commit `f21ea30`).

**Lecture** : cohérent avec, et largement prédit par, les résultats A1/A2 v2 (clés FFN quasi-orthogonales entre elles, au plancher du cosinus aléatoire) — si les jeux de clés de couches différentes n'ont aucune structure directionnelle privilégiée par rapport à un `x` donné, aucun tri naturel ne peut émerger d'un simple produit scalaire, quelle que soit l'identité de couche. **Par la table de décision déjà posée : pas d'enchaînement sur étape 2/3 tel quel.**

**Deux lectures possibles, à distinguer avant de choisir la suite** :
- (a) `x_l` ne porte réellement aucune information de profondeur séparable — un signal externe explicite (régime 3, embedding d'étape) devient nécessaire, pas seulement utile ;
- (b) l'information existe dans `x_l` mais n'est pas récupérable par un produit scalaire non entraîné contre `gate_proj` spécifiquement — un signal appris (même simple/linéaire) pourrait la récupérer, ce qui garderait le régime 4 vivant.

**Test suivant, écrit et validé synthétiquement, envoyé à experiment-manager pour exécution réelle** : `learn/indexed_attention/diagnose_layer_linear_probe.py` — un probe linéaire (`nn.Linear` seul, sans couche cachée) entraîné à prédire la couche d'origine à partir de `x_l` réel, avec séparation train/test et comparaison au plancher de hasard. Si le probe dépasse largement le hasard sur les données tenues à l'écart → lecture (b), un signal appris reste prometteur. Si le probe reste proche du hasard → lecture (a), le régime 3 (ou D) devient la voie la plus soutenue par la donnée. Bug trouvé et corrigé avant l'envoi : un paramètre `n_tokens` mort absorbait silencieusement la valeur de `--tokens_per_shard` dans l'appel `main()` — trouvé par le même test de sanité synthétique (planter un signal séparable, vérifier la détection ; planter du bruit pur, vérifier le quasi-hasard) déjà utilisé pour les deux scripts précédents.

**[RÉSULTAT 2026-09-19/20] Probe linéaire exécuté : lecture (b) fortement confirmée, `test_acc=0,9997`** (`train_acc=1,0`, `chance_floor=0,3333`, 22 773 exemples train / 9 759 test). L'information de profondeur est quasi parfaitement séparable linéairement à partir de `x_l` — mais avant de conclure au régime 4, un risque de confond restait ouvert : A1 avait déjà montré `||k_attn||` croissant fortement avec la profondeur (15,8→70,3), donc un probe sur `x_l` brut pourrait séparer les couches sur la seule **norme**, sans structure **directionnelle** exploitable par un produit scalaire (ce dont le régime 4 a réellement besoin).

**[CONTRÔLE, même session] Ablation de normalisation ajoutée à `diagnose_layer_linear_probe.py`** : même probe entraîné sur `x_l` normalisé en L2 (norme unitaire, direction seule), en parallèle de la version brute. Validé synthétiquement sur 3 cas construits (signal de norme seule → devrait échouer après normalisation ; signal directionnel seul → devrait survivre ; aucun signal → devrait rester au hasard dans les deux cas) avant renvoi à experiment-manager.

**[RÉSULTAT 2026-09-19/20] Signal directionnel confirmé, pas un artefact d'échelle** : `test_acc=0,9995` **même après normalisation L2** (`raw test_acc=0,9998`), quasi identique à la version brute. Le signal de profondeur dans `x_l` est donc bien directionnel, pas juste porté par la norme — **le régime 4 est théoriquement viable** : l'information nécessaire à un adressage appris existe et est de la forme qu'un produit scalaire peut exploiter, à condition d'une projection apprise.

**Test suivant (troisième étape de la séquence de désambiguïsation), écrit et validé synthétiquement, envoyé à experiment-manager** : `learn/indexed_attention/train_layer_router.py`. Le probe linéaire répond à une question de classification ("l'info existe-t-elle") ; ce script répond à la question de **mécanisme de retrieval** réellement pertinente pour le régime 4 : une projection de requête entraînée (`nn.Linear`, même rôle que `HierarchicalMemory.q_proj`) route-t-elle effectivement, via un vrai softmax sur la banque de clés FFN unifiée, vers la bonne région ? Objectif d'entraînement : maximiser directement la masse d'attention sur la vraie région (pas un proxy de classification générique) — la même quantité mesurée (et absente) en étape 1. Validé synthétiquement sur un cas "routage apprenable mais non aligné brut" (une rotation aléatoire fixe sépare `x` de `K`, seule une projection apprise peut l'annuler) et un cas "aucun signal" — ce dernier a aussi révélé pourquoi le split train/test est indispensable ici : une seule couche `Linear` sur-apprend le bruit pur à 96% de précision **train** avec seulement 420 exemples en `d=16`, alors que la précision **test** reste correctement au hasard.

#### Maximisation de l'occupation cluster pendant l'attente — tâches parallèles préparées **[2026-09-20, model-design, autonomie totale demandée par l'utilisateur]**

Consigne explicite : ne pas attendre séquentiellement chaque résultat avant de préparer la suite — le cluster est sous-utilisé face au calendrier serré (ICLR 09-26). Trois tâches supplémentaires écrites/validées et envoyées à experiment-manager **en parallèle** du router 3-couches déjà en cours, sans dépendance entre elles :

1. **`check_layer_composition_exact.py` (S0.5)** — S0 prouvait la conversion FFN isolée exacte (`max_abs_err=0,0`) ; ce script vérifie que l'insertion de cette conversion dans la structure Post-Norm réelle d'OLMo-2 (attention réelle inchangée + FFN convertie, résiduels/normes dans l'ordre exact) reconstruit la couche **complète** bit-à-bit — le point de contrôle nécessaire avant de faire confiance à toute composition future (étape 2 du curriculum) sur un défaut d'assemblage plutôt qu'un défaut de conversion. Validé synthétiquement (`max_abs_err=0,0` sur un `Olmo2DecoderLayer` synthétique) avant envoi.

2. **Extension à 16 couches des diagnostics déjà validés (étape 1 + probe linéaire)** — aucun nouveau code, juste `--layers 0,1,2,...,15` au lieu de `0,8,15`. `collect_layer_activations` extrait déjà toutes les couches demandées d'une seule passe avant (`output_hidden_states`), donc étendre à 16 couches ne coûte quasiment rien de plus en calcul qu'à 3 — gain d'information important (couvre toute la stratification de Geva, pas un échantillon bas/moyen/haut).

3. **Comparaison couches adjacentes vs distantes sur le router entraîné** — réutilise `train_layer_router.py` tel quel avec deux jeux de couches différents (ex. `0,1,2` adjacentes vs `0,7,15` distantes). A2 avait déjà trouvé un signal faible mais présent de plus grande similarité entre couches adjacentes (cosinus max moyen 0,12-0,19 vs 0,084-0,091) — teste si ça se traduit en **difficulté de routage** plus grande entre couches adjacentes (confusion plus probable) qu'entre couches distantes, un résultat de stratification indépendant et nouveau.

**Prévenu explicitement dans l'envoi à experiment-manager** : la version 16-couches de `train_layer_router.py` (banque unifiée de 131 072 clés, l'échelle réelle visée par S2) n'est **pas** dispatchée telle quelle — la boucle d'entraînement actuelle traite tout le jeu d'entraînement en un seul lot par epoch (pas de mini-batching), ce qui pourrait être lourd à cette échelle sur CPU sans réglage. Envoyé comme tâche de reconnaissance à petit budget (`--epochs 100 --n_docs 32 --tokens_per_shard 128`) avec consigne explicite de surveiller/interrompre si le temps mural dérape, plutôt que supposé sûr par défaut.
