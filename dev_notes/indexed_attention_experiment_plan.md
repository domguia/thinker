...
**Mise à jour (2026-09-13) — unification proposée par l'utilisateur, à retenir pour l'implémentation future** : le No-Op (ci-dessus), la largeur de récupération variable, et un "step de calcul pur" (une itération de la boucle qui ne va chercher aucune nouvelle information, ne fait que composer ce qui est déjà dans $R$/SM — pertinent pour le débat $N_{\text{step}} \gtrsim 2\text{-}4\times L$ de la Phase 1quater) **ne sont pas trois mécanismes séparés** : les trois émergent d'un seul mécanisme — un token Sentinel/No-Op appris $(\mathbf k_\emptyset, \mathbf v_\emptyset)$ ajouté comme candidat de plus dans le softmax unifié (§5.2/§8bis), sans seuil ni pénalité de parcimonie dédiés (contrairement au mécanisme à seuil $\theta$/$\lambda$ actuellement formalisé en spec §7.1, qui a un risque de collapse non résolu). Cette unification est **plus simple à implémenter** que les trois mécanismes séparés et évite le risque de collapse — à privilégier quand cette famille d'idées sera reprise, cf. spec §7.1 pour le détail.
