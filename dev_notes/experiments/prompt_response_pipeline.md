...
6. Éval qualitative continue : génération de texte (Thinker vs petits LLM de référence) sur **un échantillon fixe et petit, réutilisé à l'identique à chaque checkpoint** (pas de ré-échantillonnage) pour rendre les comparaisons comparables dans le temps -- script à écrire, alimente aussi une demande similaire reçue d'une session sœur (analyst-agent).
