...
**Implication restante** : le top-K KD précomputé pour hotpotqa_full est affecté à la position 0 -- décision de regénérer ou non à prendre avec l'utilisateur/data-gen-gh200 (coût precompute non-négligeable). Vérification empirique du fix (recharger le Teacher avec le template corrigé, confirmer que la masse sur `<think>` chute) prise en charge par data-gen-gh200 (infra Teacher/GPU déjà en place de leur côté).
...
**Pistes à explorer (pas encore lancées)** : (1) scheduled sampling / mélange teacher-forcing + free-running pendant l'entraînement -- désormais le candidat le plus motivé par l'analyse ci-dessus, coût : complexifie la boucle d'entraînement, nécessite un budget GPU non-trivial pour un ré-entraînement complet ; (2) vérifier directement la probabilité de `<think>` à l'initialisation aléatoire (step 0, avant tout entraînement) pour confirmer/infirmer l'hypothèse 3 à moindre coût avant d'engager (1).
