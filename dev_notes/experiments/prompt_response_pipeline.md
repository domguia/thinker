...
## Ablation n_register (1 vs 8) : résultat, avec un incident infra documenté (2026-09-23, suite)

Même recette KD (phase17), `--n_register 1` (flag CLI existant, pas de code à ajouter). Entraîné sur GPU L40S (Rennes, abacus26-1) après un premier essai avorté sur `grouille-2` (Nancy, besteffort) : le process mourait silencieusement pendant le chargement du dataset (aucune trace Python, aucun OOM -- cgroup max 522MB sur une limite de 62.7GB) à 2 reprises. Root cause identifiée par infra-agent : `grouille` est partagé avec un autre utilisateur (`ybardes`, projet slices-fr-misc) ayant des réservations prioritaires cycliques -- job besteffort préempté net par OAR (cgroup entier tué), PAS un problème du montage NFS cross-site (testé sain en direct). **Leçon générale : éviter `grouille`/besteffort pour un run qui ne tolère pas d'interruption silencieuse** (contrairement à un training avec `--patience`/checkpointing long, un crash sans trace au milieu du chargement de données perd tout le run). Détails infra : `dev_notes/grid5000_usage.log.md`.

`val_answer=7.220` (comparable aux autres checkpoints). **Qualitatif : 46/60 générations dégénérées** (plus élevé que le baseline KD n_register=8, cf. section "KD-vs-CE-only isolé" : 46/60 déjà pour KD à l'époque -- même ordre de grandeur, pas d'aggravation nette).

**Diagnostic de calibration : accuracy argmax teacher-forcée = 0.375%, strictement identique à n_register=8 (0.25-0.5% sur tous les checkpoints testés).** Réduire le nombre de slots de registre à 1 (au lieu de 8) ne change rien à la sévérité du collapse. Safe-bet encore plus concentré (top5=89.4%, dominé par `"Based"` 35.75% + `"1"` 27.5%).

**Conclusion : infirme cette 4e hypothèse structurelle.** Bilan complet à ce stade : n_step, KB/retrieval, answer-head-per-position (proxy), et n_register -- AUCUNE de ces 4 pistes architecturales isolées n'explique le calibration-collapse sévère et stable (~0.25-0.5%) observé sur tous les checkpoints Thinker, alors que Baseline C (dense transformer, même échelle/données) atteint 21.2%. La cause reste non identifiée parmi les composants testés individuellement -- candidat restant à explorer : une INTERACTION entre plusieurs mécanismes plutôt qu'un composant isolé, ou quelque chose de plus fondamental dans la boucle recurrente elle-même (pas juste son nombre d'itérations) que ces ablations n'isolent pas. Fichiers : `logs/divergence_nregister1.json`, `checkpoints/retrieval1_nregister1_best_qualitative.md`.

## Test combiné (answer_head_per_position + n_register=1) : dernière itération, hypothèse d'interaction infirmée -- clôture de l'investigation (2026-09-23, suite)

Décision utilisateur (via supervisor-agent) : dernier test avant d'arrêter la chasse. Combine les deux ablations dans le MÊME checkpoint (`--answer_head_per_position --n_register 1`, même recette KD/données) pour tester si le collapse vient d'une INTERACTION entre mécanismes plutôt que d'un seul isolé.

`val_answer=7.248` (comparable). **Accuracy argmax teacher-forcée = 0.4375%, toujours dans la même plage (0.25-0.5%) que chaque ablation isolée et le baseline.** Aucun effet de synergie observé. Qualitatif : pas de warning de dégénérescence sur ce run précis (variance normale entre runs, cf. les autres checkpoints qui oscillent aussi 44-50/60).

**Conclusion : infirme aussi l'hypothèse d'interaction (au moins pour cette paire de mécanismes).** Fichiers : `logs/divergence_combined_ablation.json`, `checkpoints/retrieval1_combined_ablation_best_qualitative.md`.

### Clôture de l'investigation "génération collapse" (décision utilisateur, 2026-09-23)

7 ablations/isolations testées au total (n_step récurrence, KB/retrieval, answer-head-per-position, n_register, leur combinaison, le scaling de largeur d_model 256->512 -- cf. "phase22" ci-dessus, accuracy argmax=0.4%, strictement identique -- plus le contrôle architectural Baseline C dense-transformer) : **aucune n'explique le calibration-collapse sévère et stable (accuracy argmax teacher-forcée ~0.25-0.5% partout sur Thinker, quelle que soit la taille du modèle, contre 21.2% pour Baseline C à échelle/données identiques).** Décision utilisateur : arrêter la chasse au mécanisme isolé, documenter le collapse comme une propriété du mécanisme récurrent Thinker dans son ensemble (non isolable à un composant testé individuellement ni à une simple interaction par paire), et recentrer l'effort sur les résultats de thèse centraux (extrapolation n_step, performance retrieval, efficience) pour le papier plutôt que de continuer cette investigation diagnostique.
...
## Piste de récupération EM/F1 : sampling et format court -- aucun ne fait remonter l'EM (2026-09-23, suite)

Décision utilisateur (via supervisor-agent) : tester si un changement de décodage seul (pas de training) peut récupérer une réponse correcte malgré le calibration-collapse. Deux leviers testés, mêmes 500 exemples, sur les 2 mêmes checkpoints (KD full-coverage, CE-only) :

1. **Sampling non-greedy** (température=0.7, top_p=0.9, via `--temperature`/`--top_p` ajoutés à `eval_retrieval_em_f1.py`) : EM=0.00% (KD et CE-only), F1=0.45%/0.68% -- similaire ou légèrement pire que le greedy (0.32%/0.79%).
...
## E5 (WRITING_PLAN §4, P1) : diagnostic mécanistique par itération -- convergence vers une DIRECTION fixe qui croît SANS BORNE (2026-09-23)

Question : le collapse est-il une convergence vers un point fixe/attracteur du cœur récurrent ? Nouveau script `learn/indexed_attention/diagnose_iteration_dynamics.py` -- rejoue manuellement la boucle `Thinker.forward()` (register init -> n_step appels à `model._step()`) depuis l'extérieur pour capturer `R` et `(sm_k, sm_v)` après CHAQUE itération (forward() ne renvoie que l'état final). Mesure à chaque étape : `||R||` (norme), rang effectif de `R` (participation ratio des valeurs singulières), similarité cosinus `cos(R_i, R_{i-1})`, et un "logit-lens" fidèle -- appelle l'output stream "answer" avec seulement les `i` premières entrées de `(sm_k, sm_v)` (le vrai mécanisme de lecture par cross-attention, pas une projection hypothétique de R), donnant l'entropie de ce que le modèle prédirait s'il s'arrêtait à l'itération `i`. Checkpoint : `retrieval_kd_fullcov_best.pt` (KD, couverture full), sweep jusqu'à n_step=32 (même régime que E1).

**Résultat net, 4 signaux cohérents entre eux** :
1. **`||R||` croît QUASI LINÉAIREMENT et SANS BORNE** : 63.6 (step1) -> 138.7 (step4) -> 1070.3 (step32), aucun plateau, incrément quasi constant (~30-34/step) sur toute la plage -- confirme littéralement le commentaire du code source lui-même sur `outer_norm` ("Nothing bounds ||R|| as n_step grows").
2. **Rang effectif s'effondre** : 1.30 (step1) -> 1.20 (step4) -> 1.06 (step32), tend asymptotiquement vers 1 (rang-1 pur) -- `R` devient dominé par une seule direction.
3. **`cos(R_i, R_{i-1})` -> 1.0000** rapidement (0.9994 dès step10, exactement 1.0000 à partir de step21) -- la DIRECTION de `R` cesse de changer alors que sa NORME continue de croître : ce n'est pas un point fixe classique (borné) mais une convergence vers un **rayon/direction fixe le long duquel `R` diverge sans borne**.
