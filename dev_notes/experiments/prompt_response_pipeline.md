...
**GPU utilisé** : Nancy (`graffiti-4`, RTX 2080 Ti 10.5GB, via `infra-agent` -- Rennes trop contesté ce jour). `--batch_size 32` -> OOM immédiat (vocab 248k, `topk_kd_loss` fait un `logsumexp` sur toute la distribution) ; `--batch_size 8` fonctionne. Run en cours au moment de cette note (`checkpoints/tinystories_pr_kd_best.pt`, `logs/tinystories_pr_kd_train.log`).
