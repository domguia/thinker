...
**Pistes à explorer (pas encore lancées)** : (1) scheduled sampling / mélange teacher-forcing + free-running pendant l'entraînement (réduit l'écart train/inférence, coût: complexifie la boucle d'entraînement), (2) sampling avec température/nucleus au lieu de greedy pur (un `<think>` initial dégénéré pourrait n'être qu'un mode-collapse du greedy, pas une preuve que TOUTE distribution de sortie est mauvaise -- à vérifier), (3) vérifier si le token `<think>` est simplement le token de plus haute fréquence a priori dans le vocab Qwen3.5 (auquel cas c'est un collapse vers la moyenne, symptôme classique d'un modèle sous-entraîné en régime free-running) en inspectant les logits complets à la position 0 plutôt que le seul argmax.
