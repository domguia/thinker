...
**Réserve méthodologique, à garder attachée à ce résultat (`model-design`, 2026-09-20)** : le mécanisme validé ici route correctement dans un modèle **préentraîné figé** (OLMo-2-1B), avec une projection de requête entraînée seule, en isolation, sur cette tâche de routage précise. **Ça ne démontre pas encore qu'un registre Thinker appris de zéro développerait la même capacité** dans le contexte plus riche/contraint de la boucle complète (où la requête doit aussi porter d'autres rôles — extraction, composition — pas seulement le routage mémoire). À traiter comme "le mécanisme est représentationnellement possible et appris facilement en isolation", pas comme "Thinker saura router sa mémoire correctement une fois entraîné en entier" -- cette dernière affirmation reste à tester, probablement à l'étape 2/3 du curriculum une fois la question du signal d'étape (§ ci-dessus) reliée à la construction effective d'une boucle Thinker sur ces couches.

Statut au 2026-09-20 : router 16 couches (échelle réelle S2, 131 072 clés, mini-batché) en cours d'exécution sur `paradoxe-2` (>1h20, ~49 cœurs, aucun signe de blocage) -- résultat le plus informatif restant sur ce fil, à documenter séparément dès qu'il tombe.
