...
**Correctif nécessaire (`experiment-manager`, 2026-09-13) : `seq_len=32` converge trop lentement en direct pour être abordable par cellule** — même le contrôle `read_step=6` n'avait pas fini de converger après 30 min/2700+ pas (encore en hausse, 8,7%→48,6%). Résolu par `--seq_len_curriculum "8,16,24,32"` dans `train_toy_memory.py` (même principe que le curriculum `read_step` déjà en place — voir §4.5). Un second problème est apparu avec le curriculum lui-même : un budget total unique (`--max_time_minutes`/`--max_steps`) partagé entre toutes les étapes peut être entièrement consommé par des étapes précoces lentes à converger, laissant peu ou pas de temps à l'étape cible (32) — deux runs de la même grille avaient atterri à des étapes différentes (16 et 24) pour cette raison, pas à cause d'un bug de mesure (vérifié : le rapport final étiquette toujours correctement `final_stage_seq_len`/`final_stage_read_step`, jamais la valeur cible par défaut). **Corrigé** : `--final_stage_min_steps` (défaut 1500) garantit un délai minimal réel à la dernière étape du curriculum, quitte à dépasser le budget global demandé (n'affecte pas les runs à échelle fixe, sans curriculum).
