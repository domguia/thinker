...
**[IMPLÉMENTÉ, MVP intra-batch, 2026-09-20]** — décisions prises pour cette première version (à rediscuter avant toute extension "large volume de documents" évoquée par l'utilisateur, voir note plus bas) :
- Token d'ingestion → nouvel embedding dédié, id réservé au-delà de `vocab_size` (`Thinker(use_ingest_token=True)`), pas de `tokenizer.add_special_tokens()` (aucun précédent dans ce repo, aurait demandé un resize/retrain de l'embedding du tokenizer).
- KB pendant l'ingestion → lecture activée (choix utilisateur) : `Thinker._step` (boucle factorisée hors de `forward`, réutilisée par `ingest`) appelle `self.memory.attend(R)` normalement, donc un document ingéré voit les documents déjà ingérés avant lui dans le même appel.
- Cache inter-batch → **pas implémenté dans cette version** (choix utilisateur explicite : commencer par la version minimale intra-batch, discuter ensemble de la version optimisée — réutilisation cross-batch avec `.detach()`, avant de l'implémenter). Chaque document est ré-ingéré à chaque batch.
- Code : `Thinker._step`/`Thinker.ingest` (`core/indexed_thinker_model.py`), `HierarchicalMemory.clear`/`add_static_level` (`core/indexed_memory.py`, généralise `build_static` à plusieurs niveaux, sans la contrainte `depth==0`), `Thinker.forward(kb_prebuilt=True)` pour sauter le `build()` interne, `ingest_documents()` + `--ingest_kb`/`--ingest_n_step` (`learn/indexed_attention/train_prompt_response.py`, retrieval uniquement — un document par bloc `kb_tokens` existant, pas de changement du dataset).

**Note sur la discussion "grand volume de documents" (utilisateur, 2026-09-20)** : l'utilisateur envisage un usage à plus grande échelle (des articles Wikipédia de référence pour la rédaction d'un article, avec distracteurs) nécessitant des méthodes plus avancées : (a) production des KV mem en mode inférence (sans garder le graphe du modèle) puis re-différentiation façon *gradient checkpointing* pour propager le gradient d'ingestion sans garder tout le graphe vivant ; (b) taille de KV mem variable par document (fonction du nombre de pas d'ingestion, lui-même variable) plutôt que fixe ; (c) une mesure de densité d'information par document (via analyse des embeddings) pour piloter à la fois combien de pas d'ingestion lui allouer et comment subdiviser la mémoire hiérarchique (plus de subdivision pour les zones denses) — pourrait aussi servir de signal supervisé pour un mode "rétrieval sans document de référence" (le modèle apprend à ranger dans sa KB des tokens proches du sujet traité) ; (d) dimensionnement lié à la diversité des échantillons, rapproché du problème classique du *contrastive learning*.

**[IMPLÉMENTÉ, 2026-09-20]** — (a) et (b) ci-dessus, version simple :
- **(a) gradient checkpointing** : `Thinker.ingest()` a été rendu **pur** — il ne lit plus `self.memory` (état mutable) pendant la boucle d'ingestion ; à la place, il reçoit `prior_k`/`prior_v`/`prior_mask` en arguments explicites (les documents déjà ingérés, concaténés), lus via une nouvelle méthode **stateless** `HierarchicalMemory.attend_static(query, K, V, mask)` (factorisée depuis `attend()` via `_attend_kv`). C'était une condition **nécessaire**, pas seulement une simplification : `torch.utils.checkpoint` refait le forward pendant le `backward()`, et si `ingest()` lisait encore `self.memory` (mutable, modifié entre-temps par les documents ingérés après ou par le `clear()` du batch suivant), le recompute lirait un état différent de celui du forward original → gradient silencieusement faux, sans erreur ni NaN pour le signaler. `ingest(..., use_checkpoint=True)` déclenche `torch.utils.checkpoint.checkpoint(self._ingest_impl, ..., use_reentrant=False)`. Vérifié empiriquement (script ad hoc, pas encore un test committé) : loss et gradient strictement identiques (`diff == 0.0`) avec et sans `use_checkpoint`, sur un pipeline à deux documents + passe QA complète.
- **(b) n_step variable par document** : `ingest_n_step_for()` (`learn/indexed_attention/train_prompt_response.py`) — règle simple, PAS la version densité d'information (c, toujours [OUVERT]) : `n_step = clamp(ceil(longueur_réelle_max_du_batch_pour_ce_slot / --ingest_step_size), --ingest_n_step_min, --ingest_n_step_max)`. Granularité **par slot-document, pas par exemple** (une seule valeur de n_step pour tous les exemples du batch partageant ce slot — `ingest()` traite le batch entier en un appel à itération fixe, un vrai n_step par exemple demanderait une refonte plus profonde). Flags : `--ingest_step_size` (None = désactivé, garde `--ingest_n_step` fixe), `--ingest_n_step_min`, `--ingest_n_step_max`, `--ingest_checkpoint` (active (a)).
- **(c) densité d'information** et **(d) dimensionnement/diversité type contrastive learning** : **non implémentés, toujours [OUVERT]** — fils de recherche distincts, explicitement mis de côté par l'utilisateur pour une réflexion séparée (pourrait toucher la subdivision de `HierarchicalMemory`, un signal supervisé de "où stocker", et la question de diversité des échantillons/distracteurs). Piste évoquée pour une éventuelle extension de §5 (`HierarchicalMemory`) : ne pas refondre `LevelCompressor`, mais contraindre des choix de subdivision prédéfinis (issus d'une analyse de densité par échantillon), avec une décision **souple** entre hiérarchie fixe et apprise (certains échantillons sans guidage hiérarchique du tout) plutôt qu'un remplacement rigide — à creuser.

## 8quater. Notes de recherche §8ter "grand volume de documents" — (c)/(d), non implémenté

Fil de réflexion mené avec l'utilisateur le 2026-09-20, en parallèle des expériences du MVP §8ter (via `experiment-manager`) et avec l'avis de `model-design` — rien ci-dessous n'est codé, à traiter comme une exploration à part avant toute implémentation.

### (c) Mesure de densité d'information par document

**Candidats évalués, avec compromis** :
1. **Dispersion/entropie des embeddings du document** (variance, rang effectif de PCA) — gratuit, réutilise `self.embed`, mais mesure la diversité *lexicale*, pas forcément la densité *factuelle* (un dialogue varié en vocabulaire n'a pas plus de faits réutilisables qu'un texte simple).
2. **Perplexité sous un petit modèle auxiliaire (Teacher externe)** — haute perplexité = information peu prédictible = dense. **Objection soulevée par l'utilisateur, validée dans la discussion** : la perplexité est une métrique **intrinsèquement liée à la factorisation autorégressive** d'un LM classique (produit de conditionnelles token-par-token) — un mécanisme de compression complètement différent du nôtre (registre récurrent + écriture SM, aucune prédiction autorégressive pendant l'ingestion). Ce n'est donc pas juste "corrélé mais pas identique" (nuance initiale de `model-design`) mais potentiellement une mesure **structurellement étrangère** à ce que notre mécanisme a besoin de représenter. `model-design` recommande de traiter le lien perplexité↔n_step-nécessaire comme une **hypothèse à vérifier empiriquement** (sonder à quel `ingest_step_size` la reconstruction/retrieval sature, sur des documents de perplexité connue), pas comme une supposition acquise dès le codage de l'heuristique.
3. **Ratio de compression façon gzip** — quasi gratuit, aucun modèle nécessaire (pas de problème poule/œuf), mais très grossier, sensible au formatage plutôt qu'au contenu sémantique.

**Décision provisoire** : si (c) est un jour implémenté, calcul **offline** (même pattern que `precompute_teacher_targets.py` — un script séparé precompute un score par document, stocké à côté des données, lu sans coût pendant l'entraînement) plutôt qu'en live (éviterait d'ajouter un forward-modèle-auxiliaire à chaque ingestion, ce qui irait à l'encontre de l'objectif d'efficacité déjà poursuivi par le checkpointing/n_step variable). Métrique candidate à valider empiriquement avant de choisir : privilégier une métrique architecture-agnostique (option 1 ou 3) plutôt que la perplexité d'un Teacher classique (option 2), justement à cause de l'objection ci-dessus — ou, si on garde la perplexité, la traiter explicitement comme hypothèse à tester, pas comme vérité acquise.

**Mécanisme d'implémentation privilégié** (si validé) : ne pas toucher `LevelCompressor`/`HierarchicalMemory` — piloter le nombre d'appels `Thinker.ingest()` par document (découpage en plusieurs sous-blocs pour un document dense/long, un seul appel pour un document court/peu dense), chaque sous-bloc devenant un niveau KB distinct via `add_static_level`. Extension directe de l'option (b) déjà codée (n_step par longueur), pas une nouvelle brique.

### Hiérarchie de connaissances par clustering d'embeddings — profondeur vs volume de datapoints

Question de l'utilisateur : en clustering hiérarchique multi-niveaux (ex. KNN récursif) sur les embeddings, comment faire en sorte que la **profondeur** de la hiérarchie reflète la richesse conceptuelle d'un sujet plutôt que le simple **volume de points** disponibles dans le corpus pour ce sujet ? Un k-means/clustering agglomératif classique subdivise là où il y a beaucoup de points (même s'ils sont quasi-identiques), et laisse plat un sujet rare mais réellement subtil — exactement l'inverse de ce qu'on veut.

Quatre pistes discutées :
1. **Scission par volume/dispersion interne** (diamètre ou rayon en embedding, pas comptage de points) — le plus proche de l'intuition initiale de l'utilisateur ("découpage des espaces de connaissances en fonction de leur volume"), mais se heurte à la malédiction de la dimension (les distances se concentrent en haute dimension, un seuil de rayon universel est difficile à calibrer, l'échelle sémantique naturelle diffère par domaine).
2. **Scission statistiquement justifiée** (test de sous-structure par nœud, ex. GMM à 2 composantes vs 1, critère BIC/AIC) — scinde seulement si la sous-structure est statistiquement prouvée, indépendamment du nombre de points ; un cluster dense mais unimodal (quasi-doublons) n'est PAS scindé. Plus coûteux (test par nœud), mais répond directement au problème.
3. **HDBSCAN** (ou équivalent à hiérarchie par persistance plutôt que branchement k-means imposé) — outil éprouvé conçu explicitement pour ce genre de confusion densité/structure ; ses hyperparamètres (`min_cluster_size`, `min_samples`) réintroduisent une notion de volume minimal, mais de façon explicite/réglable plutôt qu'en artefact caché.
4. **Correction a posteriori** (clustering classique + métrique de dispersion par cluster obtenu, utilisée seulement pour décider combien de subdivisions supplémentaires allouer) — le plus simple à implémenter, mais rustine plutôt que solution : la géométrie des clusters reste biaisée par le volume, seule la décision finale est corrigée.

**Recommandation (discussion, pas tranchée)** : pour l'usage immédiat de §8ter (piloter `n_step`/nombre de chunks par document), une hiérarchie de corpus globale est probablement disproportionnée tant que le lien densité→n_step-nécessaire n'est pas validé (cf. objection ci-dessus sur (c)) — repartir d'une métrique par document plus simple (option 1 ou 3 de (c), sans construire d'arbre) avant d'investir dans 2/3 comme fil de recherche séparé sur une hiérarchie de connaissances globale (utile pour une éventuelle KB persistante §8bis à plus long terme, hors scope §8ter).

### (d) Risque de collapse/raccourci si un document est trop souvent réutilisé

Analogie avec l'effondrement de représentation en apprentissage contrastif (negatives trop faciles/répétés) : si un document revient souvent (surtout une fois un cache inter-batch en place), le modèle pourrait apprendre à "reconnaître" ce document plutôt qu'à en produire un encodage généralement utile pour n'importe quelle question dessus.

Quatre pistes (ordre de préférence) :
- **D. Diagnostiquer d'abord** (généralisation question↔document sur des paires jamais vues ensemble à l'entraînement ; similarité inter-documents des KV mem au fil de l'entraînement) — **retenu en premier**, cohérent avec la discipline déjà établie sur ce projet (`model-design` : comparé au diff de code avant relance, au garde-fou LR qui épuise le sweep avant de conclure à un mur). L'utilisateur pense que de bonnes données (diversité de questions/distracteurs) pourraient déjà suffire à éviter le problème — hypothèse à tester avant toute intervention architecturale.
- A. Diversité côté données (vérifier ce que fait déjà le pipeline — pas de nouveau code de modèle).
- B. Augmentation stochastique à l'ingestion (perturbation légère du document à chaque passage) — coût quasi nul, empêche la mémorisation par position exacte.
- C. Objectif contrastif explicite sur les KV mem (type InfoNCE) — réponse "propre" mais chantier de conception à part entière (nouvelle perte, hyperparamètres de température/negatives), à n'engager que si B s'avère insuffisant.

**[OUVERT]** : tout ce qui précède (métrique de densité, hiérarchie par clustering, diagnostic de collapse) reste à discuter et n'est pas planifié dans le plan d'expérience courant — prochain point de décision quand un budget GPU sera disponible pour valider empiriquement les hypothèses de (c) plutôt que de coder à l'aveugle.
...
### 13.2 Levier immédiat : partage de poids embedding↔tête (weight tying) **[IMPLÉMENTÉ 2026-09-20]**

Question posée par l'utilisateur, en repartant directement de §13 (l'embedding/tête domine le budget de paramètres à grand vocabulaire) : y a-t-il des moyens d'accélérer/éviter l'apprentissage de la tête depuis zéro, en plus de l'initialisation depuis le Teacher (§13.1) ?

**Levier le plus simple, sans lien avec le Teacher** : partager la même matrice `(vocab, d_model)` entre `self.embed` (entrée) et `OutputStream.head` (sortie) d'un stream, plutôt que deux matrices indépendantes — pratique standard (GPT-2 et consorts), divise par ~2 le plus gros poste de paramètres immédiatement, coût de risque quasi nul. §13.1 notait que `Thinker` avait ces deux matrices **déjà séparées** ("aucune tension d'untying à gérer") pour permettre une init indépendante depuis le Teacher — mais ne les tie pas non plus par défaut ; les deux options (tie vs. init séparée depuis le Teacher) sont mutuellement exclusives sur un même stream, pas concurrentes globalement (on peut tie certains streams et initialiser séparément d'autres).

**Implémenté** (`core/indexed_thinker_model.py::Thinker`) :
- `tie_stream_embed: set` — partage `self.streams[name].head.weight` avec la table d'embedding correspondante (`self.embed` par défaut, ou `self.stream_embed[name]` si ce stream a son propre vocabulaire, §11ter) via assignation directe du même `nn.Parameter` (pas une copie de valeurs — le gradient de l'un met à jour l'autre automatiquement). Nécessite `stream_dims[name] == vocab_size` de la table ciblée, vérifié par assertion.
- `embed_init`/`freeze_embed`, `stream_embed_init`/`freeze_stream_embed`, `stream_head_init` — initialisation depuis une matrice déjà projetée (calcul offline, voir `learn/distill/extract_teacher_embed_init.py` ci-dessous), avec gel optionnel de l'entrée (pas de la tête, jamais proposé — §13.1 recommande explicitement de la garder entraînable). Geler ne réduit **pas** le calcul (la matmul `d_model→vocab` reste nécessaire pour les logits), seulement l'état optimiseur (pas de moments Adam sur cette table).
- Garde-fous explicites (pas d'erreur silencieuse) : `freeze_embed`/`freeze_stream_embed` refusent si le stream correspondant est tied (geler l'embedding gèlerait alors aussi sa tête tied, contredisant §13.1) ; `stream_head_init` refuse sur un stream tied (ambiguïté entre deux tenseurs d'init pour un seul poids partagé) ; tying + `stream_embed_init` ensemble est en revanche accepté et attendu (initialise la table partagée, la tête en hérite gratuitement).

**Script d'extraction ajouté** : `learn/distill/extract_teacher_embed_init.py` — charge un Teacher réel (même infra que `bench_teacher.py`/`precompute_teacher_targets.py`), projette `get_input_embeddings()` (et `get_output_embeddings()` si `tie_word_embeddings=False` côté Teacher, cf. §13.1's `Qwen/Qwen3.8-27B`) de `hidden_size` vers `d_model` via projection aléatoire orthogonale ou SVD/PCA tronquée (`torch.pca_lowrank`), sauvegarde en `.npz`. Nécessite un GPU pour charger le Teacher (même contrainte que le precompute KD) mais l'extraction elle-même est bon marché une fois chargé. **Rappel de contrainte, comme pour le KD** : la correspondance ligne-à-ligne (`row i = token id i`) n'a de sens que si le Thinker cible utilise **exactement le même tokenizer** que ce Teacher — la projection ne change que la dimension des features, jamais le mapping id→token entre tokenizers différents.

**Testé** (`tests/test_indexed_memory.py::TestWeightTyingAndTeacherInit`, 12 tests) : partage effectif du même `Parameter` (pas une copie), gradient d'un stream tied met bien à jour la table partagée, rejet des tailles de vocabulaire incompatibles, init/copie correcte avec/sans gel, tous les garde-fous de conflit ci-dessus. Suite complète : 93/93 passants.

**Levier de calcul (pas juste de paramètres), documenté mais pas implémenté** : ni le tying ni l'init Teacher ne réduisent le coût FLOPs de la matmul `d_model→vocab` à chaque position (nécessaire pour les logits complets, softmax/CE). Le vrai levier de calcul serait un **softmax échantillonné/negative sampling** (n'évaluer la perte que contre un sous-ensemble du vocabulaire — classe positive + K négatifs — au lieu de la matmul complète), à la manière de word2vec/AWD-LSTM. Plus invasif (change la sémantique de la perte, approximation plutôt qu'exacte), pas lancé sans discussion supplémentaire. Une **factorisation façon ALBERT** (`vocab → d_petit → d_model` au lieu de `vocab → d_model` directement) réduirait le nombre de paramètres de la table d'embedding elle-même, complémentaire au tying mais pas encore implémentée. **[OUVERT]**

### 13.3 Loss chunkée (levier mémoire, pas FLOPs) **[INFRA IMPLÉMENTÉE 2026-09-20, PAS ENCORE BRANCHÉE — priorité rebasculée sur les chantiers d'expérience en cours]**

Suite directe de la question §13.2 : le vrai plafond mesuré (`distillation.md`, sweep MFU/batch-size 500M-core 2026-09-13) n'est pas le calcul mais la **mémoire** — le batch ceiling (6 sur L40S) est identique en fp32 et bf16, dominé par le tenseur `(batch, block_size, vocab)` matérialisé pour le `logsumexp` de `topk_kd_loss`/`F.cross_entropy` sur les 248 077 tokens du vocabulaire Teacher-aligné.

**Implémenté** : `learn/distill/chunked_loss.py::chunked_ce_kd_loss()` — calcule CE+KD par blocs de tokens (pas de blocs de vocabulaire : la normalisation softmax/logsumexp est par-token, donc les blocs sont indépendants, pas d'accumulateur type online-softmax nécessaire), sous `torch.utils.checkpoint` pour ne jamais garder plus d'un bloc `(chunk_size, vocab)` en mémoire à la fois (le forward est refait au backward plutôt que conservé). Ne réduit **aucun** FLOP — le head continue de tourner en entier, juste jamais en un seul bloc — seulement le pic mémoire, au prix d'un recalcul partiel au backward. Valeur et gradient identiques au calcul non-chunké à la précision flottante près (`tests/test_chunked_loss.py`).

`OutputStream.forward()`/`Thinker.forward()` gagnent un flag `return_hidden` (défaut `False`, aucun changement de comportement) exposant l'état caché pré-tête `(B,T,d_model)` au lieu des logits, pour permettre à un appelant de router ce calcul vers `chunked_ce_kd_loss()` plutôt que de matérialiser les logits complets dans `Thinker.forward()` lui-même.

**Pas encore fait** : brancher ceci dans `train_prompt_response.py`/`train_sft.py` (flag `--loss_chunk_size` CLI, non ajouté) — mis en pause pour prioriser les chantiers de résultats en cours (diagnostic mémorisation retrieval, escalade budget noctx/retrieval, scale-up jeux complets, `use_ff`) plutôt que ce levier d'infrastructure, jugé secondaire tant que le batch_size n'est pas identifié comme le facteur limitant actuel. Les leviers de FLOPs réels (factorisation ALBERT, softmax adaptatif/échantillonné, §13.2) restent **[OUVERT]**.
...
### 14.9 Trois leviers KD portés directement sur Thinker **[IMPLÉMENTÉ 2026-09-20, exploratoire/petite échelle]**

Suite directe de §13.3 (loss chunkée) et de la Q2 du plan d'expérience (`indexed_attention_experiment_plan.md`, distillation de représentation) — décision utilisateur explicite de porter les trois DIRECTEMENT sur Thinker (`train_prompt_response.py`, `train_real_text.py`) plutôt que de prototyper d'abord sur `train_sft.py` (transformer dense) comme la Q2 le recommandait initialement, la priorité immédiate étant les runs A/B/C en cours.

1. **Loss chunkée (`--loss_chunk_size`)** : rebranchée dans `train_prompt_response.py` via `Thinker.forward(return_hidden=True)` + `chunked_ce_kd_loss()` appliqué par stream (`raw_model.streams[name].head`). Numériquement identique au chemin non-chunké (vérifié : `ce_answer`/`kd_answer` identiques à 4 décimales entre les deux chemins sur le même smoke test).
2. **Ancrage embedding (`--embed_teacher_target`/`--embed_kd_weight`)** : MSE continue entre `self.embed.weight` et une cible Teacher-projetée fixe (`extract_teacher_embed_init.py`), portée sur `train_prompt_response.py` ET `train_real_text.py` (donc disponible pour `general` aussi).
3. **KD de représentation (Q2)** : `data/prompt_response_dataset.py::PromptResponseReprTargets` — réutilise EXACTEMENT le même alignement caractère→token déjà résolu pour le KD logit (`_resolve_span`'s `(tok_start, n)`), appliqué à un precompute `--hidden_layers` de `precompute_teacher_targets.py` (déjà existant, jamais exploité avant). Projection aléatoire fixe (jamais entraînée) du hidden_size Teacher vers `--repr_proj_dim`, comparaison par perte cosinus (pas MSE brute, risque d'instabilité déjà noté en §11bis) avec un adaptateur `nn.Linear(d_model, repr_proj_dim)` entraîné côté student, et montée en poids progressive (`--repr_kd_warmup_steps`) contre le risque de géométrie latente rigide (FitNets/MiniLM, `raw/Distill-reasonning-stream.md:446`). **Choix de conception clé** : aligne les positions de la séquence de réponse (OutputStream, alignement déjà résolu), PAS le registre récurrent `R` aux couches du Teacher — évite la question non résolue de correspondance boucle↔couches signalée dans `raw/Branch-•-Indexed-Attention.md`.

Testé (`tests/test_repr_kd.py`, 9 tests + smoke tests CLI manuels retrieval/reasoning combinant les 3 leviers simultanément) : suite complète 107/107. Pas encore mesuré sur un vrai run à l'échelle — exploratoire, à lancer en parallèle des chantiers A/B/C pour orienter la suite, pas encore un résultat.
