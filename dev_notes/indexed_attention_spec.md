...
## 13. Dimensionnement d'`IndexedThinker` — synthèse (2026-09-13)
...
**4. KB persistante (Phase 10, §8bis, pas encore codée)** — budget $(K,V)$ à fixer séparément, en utilisant un FF équivalent comme référence de taille (proposition de l'utilisateur, §5.4) : si on budgète l'équivalent d'un FF de taille standard (~13,6M à $d_{\text{model}}=1024$, `ff_hidden_mult=4`) pour cette mémoire, avec $\dim(V)=d_{\text{model}}=1024$ et $\dim(K)=d_{\text{model}}/2=512$ (ratio Product-Key Memory, §5.4) : $n_{\text{slots}} \times (\dim K + \dim V) \approx 13.6\text{M} \Rightarrow n_{\text{slots}} \approx 13.6\text{M}/(512+1024) \approx 8\,800$ slots — un point de départ, à ajuster empiriquement en Phase 10, pas une valeur figée.

**Total trainable estimé, config de départ ($d_{\text{model}}=1024$, sans FF, KB persistante incluse)** : $\approx 254{,}3\text{M (embed)} + 9{,}44\text{M (cœur)} + 13{,}6\text{M (KB persistante)} + \text{quelques M (streams)} \approx \mathbf{\sim 280\text{-}290M}$ — nettement sous les ~0,8-0,9B de `Qwen3.5-0.8B`, avec la profondeur de calcul portée par `N_step` plutôt que par des paramètres supplémentaires. **Rappel du garde-fou déjà posé** : ce chiffre est une **cible espérée**, pas une hypothèse de conception validée — reste conditionné aux résultats du sweep `N_step` en cours et à la Phase 10 (pas encore implémentée).
