...
- **`Compress_θ` (§5.1)** : un unique jeu de $M$ requêtes apprises par la même instance à tous les niveaux ; les poids de pooling (un seul softmax sur les enfants) sont réutilisés à la fois pour résumer $K$ et pour agréger $V$ — une instanciation spécifique de "cross-attention Perceiver-style", plus simple qu'une paire de projections $K$/$V$ de sortie séparées. À raffiner si les tests empiriques suggèrent que $K$ et $V$ ont besoin d'être résumés différemment.
  - **Bug relevé en revue (corrigé)** : la première version ne comportait que le pooling par attention, **sans feed-forward** — un vrai bloc Perceiver a toujours attention *et* FF ; sans le FF, un nœud parent ne pouvait être qu'une combinaison convexe de ses enfants (aucune transformation non-linéaire du contenu résumé). Corrigé en ajoutant un FF résiduel (`ff_k_in/out`, `ff_v_in/out`, poids toujours partagés entre niveaux) après le pooling, sur $K$ et $V$ séparément. `LevelCompressor.pool()` isole désormais le pooling pur (utile pour tester la partie attention indépendamment du FF).
