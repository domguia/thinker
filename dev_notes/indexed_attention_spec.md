...
- Un FF ajouté dans la boucle principale d'`IndexedThinker` (`use_ff=True`) opère sur $[O_{kb}, O_{sm}, R]$ **déjà spécifiques à l'exemple courant** (ce qui a été récupéré *pour cette requête précise*) — sa fonction potentielle n'est pas de mémoriser des faits génériques mais de **composer/calculer** sur des valeurs déjà extraites (ex. l'exemple des notes manuscrites : soustraire deux dates récupérées). Ce n'est pas categoriquement la même chose qu'un FF qui stocke "Paris = capitale de la France" dans ses poids.
...
**Unification proposée (utilisateur, 2026-09-13) — remplace le mécanisme à seuil $\theta$/pénalité $\lambda$ ci-dessus par le Sentinel/No-Op du §8bis, plus simple** : plutôt qu'un réseau de seuil dédié (avec son risque de collapse non résolu), utiliser directement le **Sentinel token** déjà noté dans `raw/Branch-•-Indexed-Attention.md` (ligne 1313, cf. §8bis) — une paire $(\mathbf k_\emptyset, \mathbf v_\emptyset)$ **apprise**, insérée comme candidat de plus dans le même softmax unifié (§5.2), sans seuil ni pénalité de parcimonie séparés. Le softmax natif fait alors tout le travail, exactement dans l'esprit de la résolution implicite déjà retenue en §5.3 :
- **No-Op** : le Sentinel capte ~100% du poids d'attention quand rien de pertinent n'existe dans la KB — pas de $\mathbf 0$ explicite à coder, juste $O_{\text{mem}} \approx \mathbf v_\emptyset$ (apprise, potentiellement nulle ou quasi-nulle si le modèle apprend que c'est optimal).
- **Largeur variable** : le poids résiduel du Sentinel vs. les nœuds réels module naturellement combien de "largeur" de récupération est effectivement utilisée à ce pas — pas besoin d'un $\theta$/$w_i$ séparé, c'est la même distribution de poids softmax qui porte l'information.
- **Step "compute-only"** (raccord avec le débat $N_{\text{step}}$/§ Phase 1quater du plan) : une itération de la boucle où le Sentinel domine est, de facto, une itération qui **ne va pas chercher de nouvelle information** — elle ne fait que composer/traiter ce qui est déjà dans $R$/SM. C'est exactement le "step de calcul pur" évoqué dans la discussion sur le ratio $N_{\text{step}} \gtrsim 2\text{-}4\times L$ (aller chercher vs. traiter) — le modèle peut apprendre **lui-même**, itération par itération, quand faire l'un ou l'autre, sans qu'on ait besoin de désigner explicitement quelles itérations sont "retrieve" vs. "compute".

**Ces trois idées (No-Op, largeur variable, step compute-only) ne sont donc pas trois mécanismes séparés à implémenter — c'est un seul mécanisme (Sentinel dans le softmax unifié) dont elles sont des manifestations différentes selon le contexte.** Reste hors scope MVP pour l'instant (même raison qu'avant : valider d'abord la hiérarchie de base), mais à implémenter en priorité sur le mécanisme à seuil $\theta$ s'il faut un jour choisir entre les deux — plus simple, pas de risque de collapse par sur-pénalisation puisqu'il n'y a pas de $\lambda$ à régler.
