...
### Motivation supplémentaire : stabilisation du Looped Transformer, pas seulement lecture de sortie **[NOTES, confirmé empiriquement dans cette session]**
...
**Ce n'était qu'une hypothèse théorique jusqu'à cette session — maintenant appuyée par une observation empirique directe** : en débogant l'échec GPU rapporté par `thinker-e9` (loss bloquée au niveau du hasard à `depth=4`, 64 facts), reproduit localement, on a établi que : (1) l'échec n'est pas spécifique à la profondeur de hiérarchie (`depth=2` échoue pareil) ; (2) l'échec ne se résorbe **ni** en augmentant `d_model`/`n_register` (128/4 au lieu de 64/1) **ni** en baissant le learning rate ; (3) l'échec apparaît déjà à `n_facts=16` (pas seulement 64), et sur 3000 pas d'entraînement la loss reste plate au niveau du hasard, sans tendance d'amélioration même lente. Ce pattern (échec net, non résolu par capacité ni par LR, non progressif dans le temps) est cohérent avec un problème de **signal d'entraînement trop indirect/instable à travers la boucle récurrente**, pas un problème de capacité — exactement le phénomène anticipé par la citation ci-dessus.

**Implication pour l'implémentation actuelle** : le stream `answer` seul ne fournit qu'un gradient sur le token final, après $N_{\text{step}}$ itérations et toute la hiérarchie — un signal très en aval. Le remède documenté (alignement de couches contre un Teacher pré-entraîné) suppose un Teacher réel, absent de la tâche synthétique `kb_retrieval`. **Mais le principe s'adapte sans Teacher** : sur une tâche synthétique on connaît la vérité terrain de la récupération à chaque étape (quel fait/nœud devrait être ciblé) — on peut superviser directement l'attention/la sortie intermédiaire plutôt que seulement le token final, ce qui correspond à l'Option B ("White-Box Trajectory Distillation", déjà documentée ailleurs dans la même conversation source) appliquée ici sans Teacher, juste avec la vérité terrain synthétique. **[OUVERT]** — piste à tester en priorité avant de conclure que le mécanisme d'indexation est en cause (voir plan d'expérimentation, Phase -1).
