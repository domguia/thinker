...
**Read**: this directly answers Piste A's decision table -- **A still beats C at strictly matched gradient-step count, not just at matched wall-clock time.** The 2026-09-16 result was not purely a step-count confound: even giving C and A the exact same 1000 steps, the flat retrieval baseline (A) has lower loss than the main looped config (C) at both `d_model` tested. **The gap widens sharply at larger scale** (`d_model=128`: A-C gap = 0.38; `d_model=1024`: gap = 2.39) -- opposite of what "the loop helps more once there's capacity to use it" would predict. B (`disable_kb`) stays worst everywhere, as expected (sanity check on the harness, not itself a result).

**Read this as an early-training snapshot, not an asymptotic comparison** -- per the same caution already applied to the 2026-09-16 table (and explicitly re-flagged for this rerun): 1000 steps is very early (the original grid's own framing: "1000-5500 steps out of a 200000-step ceiling... far from converged"). A loop-based model plausibly needs more steps than a flat one to make its extra machinery pay off (more parameters/paths to coordinate before the loss curve reflects that capacity), so this table shows "who's ahead very early," not "who wins at convergence." **Per the decision table's third branch ("A bat toujours C à pas apparié")**: this is a real negative result at this budget, to be reported as such, not explained away. Budget did not allow the optional intermediate-`n_step` diagnostic cell (2/3/4) this session -- left as an open question rather than guessed at.

**Note on trajectory, not just the final number**: every cell's full loss curve (not just `final_loss`) is preserved in its `RunLogger` state file's `history` list (`progress()` calls at `--log_every 20`, i.e. every 20 steps) -- available for a proper curve comparison (does the A-vs-C gap hold steadily across the 1000 steps, or is it driven by a transient early-training effect) without rerunning anything. Not yet plotted/read in this entry; flagged as the next step before treating the table above as final for the paper.

**Both readings now exist and say different things, as expected -- report both**: wall-time-matched (2026-09-16 table, above) shows A ahead with 1.2-2x more gradient steps than C in the same time; step-matched (this entry) shows A still ahead even at equal steps. Neither is "the" result -- the paper needs both axes (steps and FLOPs/wall-time) stated explicitly, per the protocol.
