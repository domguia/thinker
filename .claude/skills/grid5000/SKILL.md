...
**`ecotaxe-2` (Nantes) suspect** (2026-09-21) : 3 lancements distincts de `precompute_teacher_targets.py` s'y sont tous bloqués en état `D` (disk sleep) avant même le premier print (`Loaded N examples`), sans jamais consommer de GPU (0% util, 0 MiB), alors que la même commande sur `ecotaxe-1` a démarré et progressé normalement à chaque fois. `oarsh` vers ce nœud a aussi été intermittent (échecs `255` transitoires, résolus au retry). Cause non confirmée (I/O NFS dégradé sur ce nœud spécifique ? autre chose) — à éviter par précaution pour du calcul tant que non réexpliqué ; `ecotaxe-1` n'a montré aucun symptôme.
