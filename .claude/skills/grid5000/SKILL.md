...
**Attendre un GPU rare (H100/H200) : réserver en standing sur plusieurs sites, pas sonder l'API en boucle.** Quand tout CC≥8.9 est occupé partout, ne pas écrire un monitor qui interroge `status.json` en boucle pour détecter une libération -- soumettre directement une réservation besteffort (`-t besteffort -t idempotent sleep <walltime_secondes>`) sur chaque cluster candidat (`abacus27`/`abacus26` Rennes, `musa` Sophia, `chicoree` Lille, `vercors17`/`kinovis` Grenoble, `hydra` Lyon en exotic) -- OAR les place lui-même en FIFO et les démarre automatiquement dès qu'un slot se libère, sans sondage actif. Un monitor léger n'a plus qu'à vérifier l'état (`Running`) de ces quelques jobs déjà soumis, pas scanner l'état de tous les nœuds de la plateforme.

**Top-K par défaut désormais 64, pas 32** (consigne utilisateur, 2026-09-21) : pour tout NOUVEAU lancement de `precompute_teacher_targets.py` à partir de maintenant, passer `--top_k 64`. Ne pas relancer rétroactivement les runs déjà en cours/terminés à K=32 (gaspillage) — s'applique aux prochains runs uniquement.

**`ecotaxe-2` (Nantes) — cause racine trouvée et résolue (2026-09-21), le nœud n'était PAS en cause.** Les 3 blocages initiaux (état `D`, 0% GPU, log vide avant même `Loaded N examples`) venaient de `~/.cache/mamba/proc/<pid>.json` : ce répertoire de suivi de process de micromamba est sur le home NFS **partagé entre tous les nœuds/sites**. Quand un `micromamba run` tourne en parallèle sur un autre nœud (ex. `ecotaxe-1`), il peut y laisser un fichier JSON vide pendant son écriture — n'importe quel autre `micromamba run` sur n'importe quel nœud qui scanne ce répertoire au même moment plante silencieusement (`critical libmamba json.exception.parse_error.101: attempting to parse an empty input`) avant même de lancer Python, ce qui explique le log totalement vide côté script cible. **Fix : `find ~/.cache/mamba/proc/ -size 0 -delete` avant de relancer si `micromamba run` échoue avec cette erreur.** Chaque nœud `ecotaxe-1`/`ecotaxe-2` a par ailleurs **3 GPU A100 80GB** chacun (pas 1) — vérifié via `oarnodes -J` (`gpudevice` 0/1/2 distincts par nœud) ; un job `oarsub -l gpu=1` n'en réserve qu'un, le reste peut être libre et réservable séparément.

**Piège memmap sparse + `stat -c%s` (2026-09-21, a causé une perte de données)** : `precompute_teacher_targets.py` pré-alloue le fichier hidden-states via `np.lib.format.open_memmap(mode="w+")`, ce qui crée un fichier **sparse** dont la taille apparente (`stat -c%s`, `ls -la`) est immédiatement égale à la taille finale, indépendamment des octets réellement écrits sur disque. **Ne jamais utiliser `stat -c%s`/`ls -la` pour estimer la progression d'un tel fichier — toujours `du -sh` (blocs réellement alloués).** Un script de garbage-collection automatique des shards bruts déjà consommés (écrit pour gérer un quota disque plein en cours de merge) a utilisé `stat -c%s`, cru le merge terminé après 30s, et supprimé des shards pas encore copiés dans le memmap → perte de shards hidden pour 1502 exemples (récupérés séparément car leur top-K était déjà fusionné dans le `.npz` final et le jsonl source intact, mais recalcul nécessaire pour les hidden states). Voir `dev_notes/grid5000_usage.log.md` pour le détail de l'incident et de la récupération.

**Quota disque NFS par site — vérifier `quota -s` avant tout merge/écriture volumineuse.** Le home Nantes a une limite dure d'environ 95GB (`limit = 97657M`) ; un merge memmap de plusieurs dizaines de GB peut l'atteindre en cours de route et faire échouer silencieusement tout autre process du même utilisateur sur n'importe quel nœud du même site (`OSError: [Errno 122] Disk quota exceeded`), même sur une simple écriture de log. Vérifier `quota -s` avant de lancer un run volumineux, et prévoir la suppression de shards déjà fusionnés (correctement, via `du` et non `stat`) comme stratégie de dégagement d'urgence plutôt que d'attendre l'échec.
