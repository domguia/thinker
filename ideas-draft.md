...
# If the architecture works, we can do knowledge distillation
If the architecture works, we would like to do knowledge distillation on reasoning models (top opensource: Qwen, DeepSeek, ...), but in order to make this model internalize the reasoning and thus reason more efficiently instead of doing it through explicit language, in particular with the separation of memory/reasoning this could be very efficient, but the ability to call memory will be essential. For lack of compute we prefer to distill rather than do everything from scratch, we could also do a bit of RL after distillation. The distillation could be done by increasing the signal by creating a stream parallel to the forward pass which will be reinforced by the reasoning and from which we could decide to decode just the final answer or the final answer + reasoning, progressively we could decrease the influence of the reasoning traces on the model. by doing additional RL without constraint on the reasoning the model could maybe develop more efficient reasoning mechanisms, although interpretability is likely to be more difficult.
Testing this model on LLMs would be good to verify the generality of the architecture, but we can extend this to other subjects such as image or audio generation by adapting the decoding a bit, the advantage of this this is that the central processing unit is generic to transformers and therefore can maybe offer the same flexibility/generalization as transformers and thus apply to a wide variety of problems
